...
function renderContact(contact) {
  const { id, name, email, phone, website, address, company, picture } = contact;
...
function filterByCity(city) {
  let filteredArray = contacts.filter((contact) => { return contact.address.city.includes(city) })
...
function filterHandler() {
  const filterButton = document.querySelector("#filterOptions")
  filterButton.addEventListener("change", (event) => {
    event.preventDefault();


    if (filterButton.value != 0) { render(filterByCity(filterButton.value)) } else { console.log(contacts); render(contacts) }
...
function loadCities(contacts) {
  const optionValue = document.querySelector("#filterOptions");
  optionValue.innerHTML = `<option value="0">-- Select a city --</option>`
  const cityArray = [];
  contacts.forEach((contact) => { if (!cityArray.some((element) => element === contact.address.city)) { cityArray.push(contact.address.city) } })

  const content = cityArray.map((city) => { return `<option value="${city}">${city}</option>` }).join("")

  optionValue.innerHTML += content;
}
...
function deleteButtonHandler() {
  ...
  deleteButton.forEach((deleteBtn) => {
    deleteBtn.addEventListener("click", (event) => {
      event.preventDefault();
      console.log(deleteBtn.parentElement.getAttribute('data-id'));
      render(deleteContact(deleteBtn.parentElement.getAttribute('data-id')));
